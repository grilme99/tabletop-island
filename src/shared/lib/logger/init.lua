...
local player = ((not IS_SERVER) and game:GetService("Players").LocalPlayer or nil)
...
if (IS_STUDIO) then
	...
	FireAnalyticsLogEvent = function(level, message, traceback, customData)
		local success, err = pcall(function()
			AnalyticsService:FireLogEvent(player, level, message, {stackTrace = traceback}, customData)
		end)
		if (not success) then
			warn(err)
		end
...
function LogItem:Log(message, customData)
	...
	if (IS_SERVER and not IS_STUDIO) then
...
function Log.new()
	...
	local nameParts = debug.info(2, "s"):split(".") --:match("([^%.]-)$")
	local name = nameParts[#nameParts];

	if (validExtensions[name]) then
		name = nameParts[#nameParts - 1] .. "-" .. name
	end
